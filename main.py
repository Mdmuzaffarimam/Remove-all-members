...
# ✅ FIXED: Flask alag thread mein - bot ke baad start hoga
...
async def scheduler_loop():
    ...
    while True:
        ...
        for chat_id, sched_time in schedules:
            if sched_time == now:
                print(f"[SCHEDULER] Triggering for {chat_id} at {now}")
                try:
                    await do_remove_all(bot, chat_id)
...
async def start(client, message):
    await message.reply_photo(
        ...
        caption=(
            ...
            "**👑 For Owner:**\n"
            "🔹 `/add` — Add chat to whitelist\n"
            "🔹 `/remove` — Remove from whitelist\n"
            "🔹 `/list` — See allowed chats\n\n"
            "**⏰ Schedule (Owner, in whitelisted chat):**\n"
            "🔹 `/schedule 23:00` — Daily auto-remove\n"
...
async def add_chat_cmd(client, message):
    ...
    if add_chat_db(target_id):
        ...
        await message.reply(f"⚠️ Already authorized.")

@bot.on_message(filters.command("remove") & filters.user(OWNER_ID))
...
@bot.on_message(filters.command("schedule") & filters.user(OWNER_ID) & (filters.group | filters.channel))
async def schedule_cmd(client, message):
    chat_id = message.chat.id
    if not is_chat_allowed(chat_id):
        return await message.reply(f"❌ Chat `{chat_id}` not whitelisted. Use `/add` first.")
    ...
    if arg == "cancel":
        if del_schedule(chat_id):
            return await message.reply("✅ Schedule cancelled.")
        return await message.reply("⚠️ No schedule found.")
    if arg == "status":
        t = get_schedule(chat_id)
        if t:
            return await message.reply(f"📅 **Schedule:** `{t}` daily")
        return await message.reply("📭 No schedule set.")
    try:
        datetime.strptime(arg, "%H:%M")
    except ValueError:
        return await message.reply("❌ Invalid format! Use: `/schedule 23:00`")
    set_schedule(chat_id, arg)
    await message.reply(f"✅ **Schedule Set!**\n⏰ Daily at **{arg}**\n\nCancel: `/schedule cancel`")

@bot.on_message(filters.command(["remove_all", "banall"]) & (filters.group | filters.channel))
async def remove_all_handler(client, message):
    chat_id = message.chat.id
    if not is_chat_allowed(chat_id):
        return await message.reply(f"❌ Chat `{chat_id}` not whitelisted!")
...
@bot.on_callback_query(filters.regex(r"^ban_(yes|no)_"))
...
# ✅ FIXED: Flask alag thread mein, bot properly start hoga
async def main():
