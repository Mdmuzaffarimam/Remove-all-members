# Description: A simple Telegram Bot to remove all members from a group.
# By: MrTamilKiD
# Updates: "For more updates join @KR_BotX"
# Created on: 2025-03-07
# Last Updated: 2025-03-07
# Modified: Admin gets notice on /start

import asyncio
import os
import threading
from os import environ
from datetime import datetime

from flask import Flask
from pyrogram import Client, filters, enums
from pyrogram.types import InlineKeyboardButton as Button, InlineKeyboardMarkup as Markup
from pyrogram.errors import FloodWait, RPCError


# =========================
# ðŸŒ FLASK WEB SERVER (FOR KOYEB)
# =========================
flask_app = Flask(__name__)

@flask_app.route("/")
def home():
    return "Bot is alive!", 200


def run_flask():
    port = int(os.environ.get("PORT", 8080))
    flask_app.run(
        host="0.0.0.0",
        port=port,
        debug=False,
        use_reloader=False
    )

threading.Thread(target=run_flask, daemon=True).start()


# =========================
# ðŸ¤– TELEGRAM BOT CONFIG
# =========================
API_ID = int(environ.get("API_ID", 31943015))
API_HASH = environ.get("API_HASH", "")
BOT_TOKEN = environ.get("BOT_TOKEN", "")

ADMIN_ID = int(environ.get("OWNER_ID", "8512604416"))  # ðŸ‘ˆ apni Telegram ID

UNBAN_USERS = environ.get("UNBAN_USERS", "True") == "True"
BAN_CMD = ["remove_all", "removeall", "banall", "ban_all"]

app = Client(
    "bot",
    api_id=API_ID,
    api_hash=API_HASH,
    bot_token=BOT_TOKEN
)


# =========================
# ðŸ“Œ START COMMAND
# =========================
@app.on_message(filters.command("start") & filters.private)
async def start(client, message):
    user = message.from_user

    await message.reply(
        "ðŸ‘‹ Hi! I'm a Group Management Bot!\n\n"
        "âœ¨ What I can do:\n"
        "ðŸš« Remove all members from a group\n\n"
        "ðŸ“ How to use me:\n"
        "1ï¸âƒ£ Add me as admin in your group\n"
        "2ï¸âƒ£ Give me 'Ban Users' permission\n"
        "3ï¸âƒ£ Use /remove_all command\n\n"
        "âš ï¸ Important: I need 'Ban Users' permission to work!",
        reply_markup=Markup(
            [
                [
                    Button("ðŸ‘¨â€ðŸ’» Developer", url="https://t.me/mimam_officialx"),
                    Button("ðŸ’¬ Support", url="https://t.me/MRN_Chat_Group"),
                ],
                [Button("â­ Source Code", url="https://papajiurl.com/rryy3p")],
            ]
        ),
        quote=True,
        disable_web_page_preview=True,
    )

    # ðŸ”” ADMIN NOTICE
    notice = f"""
ðŸš€ <b>Bot Started</b>

ðŸ‘¤ Name: {user.first_name}
ðŸ†” User ID: <code>{user.id}</code>
ðŸ”— Username: @{user.username if user.username else 'No username'}
ðŸ•’ Time: {datetime.now().strftime('%d-%m-%Y %H:%M:%S')}
"""
    try:
        await client.send_message(
            chat_id=ADMIN_ID,
            text=notice,
            parse_mode="html"
        )
    except Exception:
        pass


# =========================
# ðŸ“Œ HELP COMMAND
# =========================
@app.on_message(filters.command("help") & filters.private)
async def help(client, message):
    await message.reply(
        "ðŸ¤– Simple Bot Guide:\n\n"
        "ðŸ“ Commands:\n"
        "/remove_all - Remove everyone from group\n\n"
        "ðŸ“Œ Quick Setup:\n"
        "1ï¸âƒ£ Make me admin\n"
        "2ï¸âƒ£ Give 'Ban Users' permission\n"
        "3ï¸âƒ£ That's it!",
        disable_web_page_preview=True,
        quote=True,
    )


# =========================
# ðŸš« REMOVE ALL USERS
# =========================
@app.on_message(filters.command(BAN_CMD) & (filters.group | filters.channel))
async def remove_all_users(client, message):
    chat_id = message.chat.id

    bot_admin = await client.get_chat_member(chat_id, "me")
    if not bot_admin.privileges or not bot_admin.privileges.can_restrict_members:
        await message.reply("ðŸš¨ I need 'Ban Users' permission to remove members!")
        return

    count = 0
    update_message = await message.reply(
        "ðŸ”„ Starting to remove members...\n\nâŒ› Please wait...\n\nðŸ”¹ Progress: 0",
        quote=True
    )

    async for member in client.get_chat_members(chat_id):
        if member.status in (
            enums.ChatMemberStatus.ADMINISTRATOR,
            enums.ChatMemberStatus.OWNER
        ):
            continue

        try:
            await client.ban_chat_member(chat_id, member.user.id)
            count += 1

            if count % 10 == 0:
                await update_message.edit(
                    f"ðŸ”„ Progress Update:\n\nâœ… Members removed: {count}"
                )

        except FloodWait as e:
            await asyncio.sleep(e.value)
        except RPCError:
            pass

    if UNBAN_USERS:
        async for member in client.get_chat_members(
            chat_id, filter=enums.ChatMembersFilter.BANNED
        ):
            try:
                await client.unban_chat_member(chat_id, member.user.id)
            except FloodWait as e:
                await asyncio.sleep(e.value)
            except RPCError:
                pass

    await update_message.edit(
        f"ðŸŽ‰ Operation Complete!\n\n"
        f"ðŸ‘¥ Total Members Removed: {count}"
    )


# =========================
# ðŸš€ RUN BOT
# =========================
if __name__ == "__main__":
    print("Bot + Web Server running...")
    app.run()
